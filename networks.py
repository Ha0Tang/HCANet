...
class XingBlock(nn.Module):
    def __init__(self):
        ...
        self.query_conv = nn.Conv2d(in_channels=512, out_channels=512//8, kernel_size=1)
        self.key_conv = nn.Conv2d(in_channels=512, out_channels=512//8, kernel_size=1)
        self.value_conv = nn.Conv2d(in_channels=512, out_channels=192, kernel_size=1)
        self.gamma = nn.Parameter(torch.zeros(1))

        self.softmax = nn.Softmax(dim=-1)
    
    def forward(self, x1, x2):
        b, c, h, w = x1.size()    # [N, 512, 16, 12]
        
        proj_query = self.query_conv(x1).view(b, -1, w*h).permute(0, 2, 1)  # [N, 192, 64]
        proj_key   = self.key_conv(x2).view(b, -1, w*h)                     # [N, 64, 192]

        energy = torch.bmm(proj_query, proj_key)                            # [N, 192, 192]
        attention = self.softmax(energy)                        

        proj_value = self.value_conv(x1).view(b, -1, h*w)                   # [N, 192, 192]

        x1_out = torch.bmm(proj_value, attention.permute(0, 2, 1))          # [N, 192, 192]
        x1_out = x1_out.view(b, -1, h, w)                                   # [N, 192, 16, 12]


        proj_query = self.query_conv(x2).view(b, -1, h*w).permute(0, 2, 1)  # [N, 192, 64]
        proj_key   = self.key_conv(x1).view(b, -1, w*h)                     # [N, 64, 192]

        energy = torch.bmm(proj_query, proj_key)                            # [N, 192, 192]
        attention = self.softmax(energy)

        proj_value = self.value_conv(x2).view(b, -1, h*w)                   # [N, 192, 192]
        
        x2_out = torch.bmm(proj_value, attention)                           # [N, 192, 192]
        x2_out = x2_out.view(b, -1, h, w)                                   # [N, 192, 16, 12]

        correlation_tensor = x1_out + x2_out
...
class XingBlockTOM(nn.Module):
    def __init__(self):
        super(XingBlockTOM, self).__init__()

        self.query_conv1 = nn.Conv2d(in_channels=22, out_channels=32, kernel_size=1)
        self.query_conv2 = nn.Conv2d(in_channels=4, out_channels=32, kernel_size=1)

        self.key_conv1 = nn.Conv2d(in_channels=4, out_channels=32, kernel_size=1)
        self.key_conv2 = nn.Conv2d(in_channels=22, out_channels=32, kernel_size=1)

        self.value_conv1 = nn.Conv2d(in_channels=22, out_channels=26, kernel_size=1)
        self.value_conv2 = nn.Conv2d(in_channels=4, out_channels=26, kernel_size=1)
    ...
    def forward(self, x1, x2):
        #print('x1',x1.size())
        down_x1 = interpolate(x1, [256//2, 192//2])
        #print('down_x1',down_x1.size())
        down_x2 = interpolate(x2, [256//2, 192//2])
        ...
        x1_out = torch.bmm(proj_value, attention.permute(0, 2, 1))           # [N, 26, 256*192]
        #print('x1_out',x1_out.size())
        x1_out = x1_out.view(b, -1, h, w)                                    # [N, 26, 256, 192]
        x1_out = self.upsample(x1_out)
...
class UnetGenerator(nn.Module):
    ...
    def forward(self, input):
        # INPUT_SIZE = [N, 26, 256, 192]
        agnostic_feature = input[:, 0:22, :, :]   #[N, 22, 256, 192]
...
class GMM(nn.Module):
    ...
    def forward(self, inputA, inputB):
        ...
        correlation = self.xingRelation(featureA, featureB)
